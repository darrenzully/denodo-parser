...
from pydenodo.models.vqlModels import VqlService
...
def parse_create(vql_str: str) -> list[VqlService]:
    pattern_compiled = re.compile(pattern_create)

    result = []
    for id, match in enumerate(pattern_compiled.finditer(vql_str)):
        if match.group(2) == "WEBSERVICE":
            result.append(VqlService(id, match.group(3), match.group(0)))

    return result

def parse_undeploy(vql_str: str) -> list[VqlService]:
    pattern_compiled = re.compile(pattern_undeploy_service)

    result = []
    for id, match in enumerate(pattern_compiled.finditer(vql_str)):
        if match.group(1) == "WEBSERVICE":
            result.append(VqlService(id, match.group(2), match.group(0)))

    return result

def parse_deploy(vql_str: str) -> list[VqlService]:
